const assert = require("assert/strict");
const fs = require("fs");
const path = require("path");

const root = path.join(__dirname, "..");
assert.equal(
  fs.existsSync(path.join(root, "src/app/methodology/page.jsx")),
  false,
  "Methodology must not be a public route",
);
assert.equal(
  fs.existsSync(path.join(root, "public/llms.txt")),
  false,
  "AI-facing llms.txt must not be published",
);
const visibleFiles = [
  "src/app/collections/page.jsx",
  "src/app/collections/[slug]/page.jsx",
  "src/components/HomeClient.jsx",
  "src/components/Footer.jsx",
  "src/app/video/[...slug]/VideoPlayerClient.jsx",
  "src/app/trends/weekly/page.jsx",
  "src/app/not-found.jsx",
];
const bannedPhrases = [
  "quality gate",
  "growing collection",
  "publication threshold",
  "verified videos",
  "curated adult videos",
  "verified discovery",
  "why this video was selected",
  "discovery score",
  "source views",
  "source rating",
  "catalog sync",
  "generated by ai",
  "system maintenance",
  "limit exceeded",
  "view original source on eporner",
  "saved on this device",
  "continue discovering",
  "recommended from local history",
];

for (const relativePath of visibleFiles) {
  const source = fs
    .readFileSync(path.join(root, relativePath), "utf8")
    .toLowerCase();
  for (const phrase of bannedPhrases) {
    assert.equal(
      source.includes(phrase),
      false,
      `${relativePath} exposes internal copy: ${phrase}`,
    );
  }
}

const collectionPage = fs.readFileSync(
  path.join(root, "src/app/collections/[slug]/page.jsx"),
  "utf8",
);
assert.equal(
  collectionPage.includes("collection.editorialIntro"),
  false,
  "Internal editorial text must not be rendered",
);
assert.equal(
  collectionPage.includes("collection.intent"),
  false,
  "Internal collection intent must not be rendered",
);
const footer = fs.readFileSync(
  path.join(root, "src/components/Footer.jsx"),
  "utf8",
);
assert.equal(
  footer.includes("/methodology"),
  false,
  "Footer must not link to the removed methodology page",
);

const savedPage = fs.readFileSync(
  path.join(root, "src/app/saved/page.jsx"),
  "utf8",
);
assert.equal(
  savedPage.includes('robots: "noindex, follow"'),
  true,
  "Personal saved-video page must remain outside the search index",
);

const navbar = fs.readFileSync(
  path.join(root, "src/components/Navbar.jsx"),
  "utf8",
);
assert.equal(navbar.includes("path: '/saved'"), true, "Navbar must link to Saved");

console.log("User-facing copy regression tests passed.");
